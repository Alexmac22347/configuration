...
static const char *const iec_symbols[MAX_EXPONENT + 1] = {"", "KiB", "MiB", "GiB", "TiB"};
static const char *const si_symbols[MAX_EXPONENT + 1] = {"", "kB", "MB", "GB", "TB"};
static const char *const custom_symbols[MAX_EXPONENT + 1] = {"", "KB", "MB", "GB", "TB"};
static const char *const no_symbols[MAX_EXPONENT + 1] = {"", "", "", "", ""};
...
static int format_bytes(char *outwalk, uint64_t bytes, uint64_t base, const char *const symbols[]) {
    ...
    return sprintf(outwalk, "%.1f %s", size, symbols[exponent]);
}
...
static int print_bytes_human(char *outwalk, uint64_t bytes, const char *prefix_type) {
    ...
    } else if (strcasecmp(prefix_type, "none_binary") == 0) {
        return format_bytes(outwalk, bytes, BINARY_BASE, no_symbols);
    } else if (strcasecmp(prefix_type, "none_decimal") == 0) {
        return format_bytes(outwalk, bytes, DECIMAL_BASE, no_symbols);
    } else {
